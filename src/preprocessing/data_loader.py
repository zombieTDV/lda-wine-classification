...
data_loader.py
===============
Data loading and preprocessing module for Wine classification.

This module handles:
- Loading wine data from CSV or embedded data
- Train/test splitting with seeded randomization
- Data standardization (Z-score normalization)
- Exploratory Data Analysis (EDA) output

All operations are built from scratch without external libraries like pandas/numpy.
...
# ─── Wine Column Names and Feature List ────────────────────────────────────
# These constants define the structure of the wine dataset.
# Each wine sample has a class label (1, 2, or 3) followed by 13 chemical features.

WINE_HEADER = [
    "class",
    "Alcohol",                # Alcohol percentage by volume
    "Malic_acid",            # Malic acid content
    "Ash",                   # Ash content (mineral residue)
    "Alcalinity_of_ash",     # Alkalinity level of ash
    "Magnesium",             # Magnesium concentration
    "Total_phenols",         # Total phenolic compounds
    "Flavanoids",            # Flavanoid compounds (type of phenol)
    "Nonflavanoid_phenols",  # Non-flavanoid phenolic compounds
    "Proanthocyanins",       # Proanthocyanidin content
    "Color_intensity",       # Wine color intensity
    "Hue",                   # Wine hue (color shade)
    "OD280_OD315",           # Optical density ratio (protein indicator)
    "Proline",               # Proline amino acid concentration
]

FEATURE_NAMES = WINE_HEADER[1:]  # Extract only the 13 feature names (skip 'class')


def _fetch_wine_builtin():
    """
    Return built-in wine dataset embedded in source code.
    
    This function provides a fallback data source when no CSV file is available.
    The dataset contains all 178 wine samples with their 13 chemical features.
    Each line is formatted as: class, feature1, feature2, ..., feature13
    
    Returns
    -------
    X : list[list[float]]
        Feature matrix of shape (178, 13).
        Each row = one wine sample, each column = one feature value
    y : list[int]
        Class labels (1, 2, or 3) for each sample.
        These are the target values for classification.
        
    Note
    ----
    This data is hard-coded for convenience. In production, you should use
    the CSV version from data/processed/wine.csv instead.
    """
    # Raw data string containing all 178 wine samples
    # Format: class,alcohol,malic_acid,ash,...,proline (14 values per line)
    RAW = """1,14.23,1.71,2.43,15.6,127,2.8,3.06,0.28,2.29,5.64,1.04,3.92,1065
...
def load_wine(filepath=None):
    """
    Load wine dataset from CSV file or use built-in embedded data.
    
    This function is the main entry point for loading wine data. It tries to read
    from a CSV file first. If the file doesn't exist, it falls back to using
    embedded data that's hard-coded in the module.
    
    Parameters
    ----------
    filepath : str, optional
        Path to wine.csv file. If None or file doesn't exist, uses built-in data.
        Expected format: comma-separated with header row
        
    Returns
    -------
    X : list[list[float]]
        Feature matrix of shape (n_samples, 13)
        Each row represents one wine sample
        Each column represents one chemical feature
        
    y : list[int]
        Class labels {1, 2, 3} indicating wine type
        Length: n_samples
        
    feature_names : list[str]
        Names of the 13 features for reference during analysis
        Useful for visualization and interpretation
        
    Example
    -------
    X, y, names = load_wine("data/processed/wine.csv")
        ...
        with open(filepath, newline="") as f:
            ...
            for row in reader:
                if not row:  # Skip empty lines
                    continue
                y.append(int(row[0]))  # First column = class label
                X.append([float(v) for v in row[1:]])  # Remaining columns = features
        print(f"[DataLoader] Loaded {len(X)} samples from {filepath}")
    else:
        # ── Fallback to embedded data ──────────────────────────────────────
        X, y = _fetch_wine_builtin()
        print(f"[DataLoader] Using built-in wine data — {len(X)} samples")

    return X, y, FEATURE_NAMES


def prepare_data(raw_path="data/raw/wine.data", output_path="data/processed/wine.csv"):
    """
    Convert raw wine.data (no headers) to standardized CSV with column headers.
    
    This function performs a one-time data preparation task:
    - Reads raw wine.data file which has no header
    - Adds proper column headers using WINE_HEADER
    - Writes standardized CSV to processed/wine.csv for future use
    
    This ensures the data is properly structured and easy to understand.
    
    Parameters
    ----------
    raw_path : str
        Path to raw wine.data file (source, no headers)
        Default: "data/raw/wine.data"
        
    output_path : str
        Path where to save the processed CSV (with headers)
        Default: "data/processed/wine.csv"
        
    Returns
    -------
    bool
        True if conversion successful, False if raw file not found
        
    Example
    -------
    prepare_data()  # Converts data/raw/wine.data → data/processed/wine.csv
    """
    ...
    # Read raw data and write with headers
    with open(raw_path, "r") as infile:
        with open(output_path, "w", newline="") as outfile:
            writer = csv.writer(outfile)
            # Write header row with column names
            writer.writerow(WINE_HEADER)
            # Write each data row from raw file
            for line in infile:
                line = line.strip()
                if line:  # Skip empty lines
                    writer.writerow(line.split(","))
    
    # Count processed rows for feedback
    with open(output_path, "r") as f:
        n_rows = sum(1 for _ in f) - 1  # Subtract 1 to exclude header
    
    print(f"[Prepare] Converted {raw_path} → {output_path}")
    print(f"[Prepare] {n_rows} samples, {len(WINE_HEADER)} columns")
    return True
...
def train_test_split(X, y, test_size=0.3, seed=42):
    """
    Split dataset into training and testing subsets with fixed randomization.
    
    This function randomly divides the data into two parts:
    - Training set: used to train the model (typically 70% of data)
    - Test set: used to evaluate model performance (typically 30% of data)
    
    The randomization is controlled by a seed for reproducibility.
    ...
    test_size : float, default=0.3
        Fraction of data to use as test set (between 0 and 1)
        Example: 0.3 means 30% test, 70% train
    ...
    X_train : list[list[float]]
        Feature matrix for training (70% of original data)
        
    X_test : list[list[float]]
        Feature matrix for testing (30% of original data)
    ...
    print(f"Train: {len(X_train)}  Test: {len(X_test)}")  # Output: Train: 133  Test: 45
    
    Note
    ----
    Uses Linear Congruential Generator (LCG) for pseudo-random shuffling.
    All operations done from scratch without numpy.
    """
    n = len(X)
    indices = list(range(n))  # [0, 1, 2, ..., n-1]

    # ── Fisher-Yates shuffle using LCG (Linear Congruential Generator) ────
    # This creates a deterministic random permutation controlled by seed
    rng = seed  # Initialize random number generator
    for i in range(n - 1, 0, -1):
        # Generate next pseudo-random number
        rng = (rng * 1664525 + 1013904223) % (2**32)
        # Pick random position j to swap with position i
        j = rng % (i + 1)
        # Swap
        indices[i], indices[j] = indices[j], indices[i]

    # ── Split shuffled indices into train and test ─────────────────────────
    split = int(n * (1 - test_size))  # How many samples go to training?
    train_idx = indices[:split]       # First part = train
    test_idx  = indices[split:]       # Rest = test

    # ── Gather samples and labels according to split ─────────────────────
...
def standardize(X_train, X_test=None):
    ...
    - Standard deviation = 1 (uniform scale)
    
    Formula: x_std = (x - mean) / std
    
    This is critical for machine learning algorithms, especially LDA, because:
    1. Features with large values don't dominate the learning
    2. All features are on the same scale
    3. The model learns more stable parameters
    
    Parameters
    ----------
    X_train : list[list[float]]
        Training feature matrix. Statistics are computed from this.
        
    X_test : list[list[float]], optional
        Test feature matrix to standardize. Uses training statistics.
        IMPORTANT: Must use train statistics, not test statistics!
        (Otherwise would use test information during training)
        
    Returns
    -------
    X_train_scaled : list[list[float]]
        Standardized training data
        
    X_test_scaled or (means, stds) : depends on input
        If X_test provided:
            return (X_train_scaled, X_test_scaled, means, stds)
        Else:
            return (X_train_scaled, means, stds)
            
        means: list[float] - mean value for each feature
        stds: list[float] - standard deviation for each feature
    ...
    Note
    ----
    Uses Bessel's correction (divide by n, not n-1) for efficiency.
    Handles zero-variance features by keeping them unchanged.
    """
    ...
    # ── Handle zero-variance features (std = 0) ──────────────────────────
    # If std is too small, set to 1.0 to avoid division by zero
    stds = [s if s > 1e-10 else 1.0 for s in stds]

    # ── Define scaling function ───────────────────────────────────────────
    def scale(X):
        """Apply Z-score transformation to dataset X"""
        return [[(xi[j] - means[j]) / stds[j] for j in range(d)] for xi in X]

    # ── Apply scaling ────────────────────────────────────────────────────
    X_train_scaled = scale(X_train)
    
    if X_test is not None:
        return X_train_scaled, scale(X_test), means, stds
    return X_train_scaled, means, stds


def describe(X, y, feature_names):
    """
    Print exploratory data analysis (EDA) summary.
    
    Displays comprehensive overview of the dataset including:
    ...
    This helps understand the data before training the model.
    ...
    classes = sorted(set(y))  # Unique class labels
    
    # ── Print header ────────────────────────────────────────────────────
    print(f"\n{'='*55}")
    print(f"  Wine Dataset — {n} samples, {d} features, {len(classes)} classes")
    print(f"  Classes: {classes}")
    
    # ── Count samples per class ────────────────────────────────────────
    from collections import Counter
    counts = dict(Counter(y))  # Count occurrences of each class
    ...
    for j, name in enumerate(feature_names):
        # Extract j-th feature from all samples
        col = [X[i][j] for i in range(n)]
        # Print: feature_name | min_value | max_value | mean_value
        print(f"  {name:<28} {min(col):>8.2f} {max(col):>8.2f} {sum(col)/n:>8.2f}")
